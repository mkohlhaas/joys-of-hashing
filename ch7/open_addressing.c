...
empty_table (unsigned int size, float rehash_factor)
{
  ...
  table->table             = malloc (size * sizeof (struct bin));
...
void
insert_key (struct hash_table *table, uint32_t key)
{
...
static void
insert_key_internal (struct hash_table *table, uint32_t key)
{
...
bool
contains_key (struct hash_table *table, uint32_t key)
{
  table->operations_since_rehash++;
  if (table->operations_since_rehash > table->probe_limit)
    {
...
void
delete_key (struct hash_table *table, uint32_t key)
{
  table->operations_since_rehash++;
  if (table->operations_since_rehash > table->probe_limit)
    {
...
resize (struct hash_table *table, unsigned int new_size)
{
  ...
  table->table    = malloc (new_size * sizeof (struct bin));
...
static void
rehash (struct hash_table *table)
{
  struct bin *old_bins = table->table;
  table->table         = malloc (table->size * sizeof (struct bin));
