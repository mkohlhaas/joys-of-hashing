...
static inline void
set_table_masks (hash_table_t *table)
{
...
static inline uint32_t
tables_idx_from_table (hash_table_t *table, uint32_t x)
{
  uint32_t old_masked_key = x & table->old_tables_mask;
  uint32_t tmask          = (table->split_count <= old_masked_key) ? table->old_tables_mask : table->new_tables_mask;
...
empty_table (size_t table_bits)
{
  ...
  for (uint32_t i = 0; i < no_tables; ++i)
    {
      table->tables[i] = malloc (size * sizeof (linked_list_t));
    }
...
void
new_delete_linked_list (linked_list_t *list)
{
  while (list)
    {
...
static void
grow_tables (hash_table_t *table)
{
  table->split_count = 0;
  table->k++;
  set_table_masks (table);
  ...
  for (uint32_t i = old_no_tables; i < new_no_tables; ++i)
    {
      new_tables[i] = malloc (size * sizeof (linked_list_t));
    }
  table->allocated_tables *= 2;
...
split_bin (hash_table_t *table)
{
  ...
  linked_list_t *old_bin = &table->tables[old_t_idx][s_idx];
  linked_list_t *new_bin = &table->tables[new_t_idx][s_idx];
  new_bin->next          = 0;
  table->max_bin_initialised++;

  uint32_t ti_mask = 1 << (table->k + table->table_bits);

  linked_list_t *list = old_bin->next; // save link to old
...
static void
shrink_tables (hash_table_t *table)
{
...
merge_bin (hash_table_t *table)
{
  ...
  linked_list_t *list = new_bin->next;
  while (list)
    {
      linked_list_t *next = list->next;
      list->next          = old_bin->next;
